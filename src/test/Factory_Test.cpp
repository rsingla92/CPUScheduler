...
int main(int argc, char* argv[]){
    ...
    Factory AlgFac = Factory();
    loadFile = parseTextFile();
    
    Algorithm* testAlg = AlgFac.factory_makeAlgorithm("TSP", loadFile, 3);
    testAlg->run();
    
    testAlg = AlgFac.factory_makeAlgorithm("RR",loadFile, 3);
    testAlg->run();
    
    testAlg = AlgFac.factory_makeAlgorithm("NPP", loadFile, 3);
    testAlg->run();
    
    testAlg = AlgFac.factory_makeAlgorithm("blahblahblah", loadFile, 3);
    testAlg->run();
